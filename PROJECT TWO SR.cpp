...
vector<Course>LoadDataStructure()
{
    ...
    while(1)
    {
        ...
        break;
        Course course;
        vector<string>tokenizeInformation=tokenize(line,",");
        course.courseNumber=tokenizeInformation[0];
        course.name=tokenizeInformation[1];
        for(int i = 2; i<tokenizeInformation.size();i++)
        {
            course.prerequisites.push_back(tokenizeInformation[i]);
        }
